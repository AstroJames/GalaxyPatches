...
subroutine make_boundary_hydro(ilevel)
  ...
  real(dp),dimension(1:nvector,1:ndim),save::xx_ref
  ...
  do ibound=1,nboundary
     ...
     ! Custom boundary
     if(boundary_type(ibound)==21)alt(1:8)=-(/2,1,2,1,2,1,2,1/)
     if(boundary_type(ibound)==22)alt(1:8)=+(/1,2,1,2,1,2,1,2/)
     if(boundary_type(ibound)==23)alt(1:8)=-(/1,1,2,2,1,1,2,2/)
     if(boundary_type(ibound)==24)alt(1:8)=+(/2,2,1,1,2,2,1,1/)
     if(boundary_type(ibound)==25)alt(1:8)=-(/1,1,1,1,2,2,2,2/)
     if(boundary_type(ibound)==26)alt(1:8)=+(/2,2,2,2,1,1,1,1/)
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           if((boundary_type(ibound)/10).ne.2)then
              ...
              call boundana(xx,uu,ind_cell,xx_ref,ind_cell_ref,dx_loc,ibound,ngrid)
              ...
              ! Enforce hydrostatic equilibrium
              !if(poisson.and.gravity_type==2)then
              !   ivar=ndim+2
              !   do i=1,ngrid
              !      uu(i,ivar)=uold(ind_cell_ref(i),ivar)
              !   end do
              !   do i=1,ngrid
              !      uold(ind_cell(i),ivar)=uu(i,ivar)+ &
              !           & uold(ind_cell_ref(i),1)*f(ind_cell_ref(i),3)* &
              !           & dx_loc*alt(ind)/(gamma-1.0d0)
              !   end do
              !end if
