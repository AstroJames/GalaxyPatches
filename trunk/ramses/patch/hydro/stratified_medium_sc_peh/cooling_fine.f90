...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  do ind=1,twotondim
	...
	r(i) = r(i)*scale_l
     ...
     if(cooling)then
        ...
        do i=1,nleaf
           ...
           z_leaf(i)=z_leaf(i)*scale ! in code units
        end do
	...
	! provide r and LAGN in cgs units here.
        ...
        ! added zcoord to compute photoelectric heating rate
	call solve_cooling(nH,T2,Zsolar,boost,dtcool,delta_T2,nleaf,LAGN,r,z_leaf)

        do i=1,nleaf
           z_leaf(i)=z_leaf(i)/scale ! back to grid units
        end do
     endif
