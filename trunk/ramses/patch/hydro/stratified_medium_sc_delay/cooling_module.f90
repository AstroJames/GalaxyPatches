...
module cooling_module
  ...
  real(kind=8),parameter ::sigmaT  = 6.6524d-25
  ...
  real(kind=8),parameter ::me      = 9.10938215d-27
  real(kind=8),parameter ::mu_mol  = 1.2195D0 ! neutral
  ...
  real(kind=8),parameter:: T2_max_fix=1.d+11 !  increased this
...
!=======================================================================
  ...
  do while (aexp < aend)
     ...
     do while (err_T2 > 1.d-10.and.niter <= 100)
        ...
        call cmp_cooling(T2,nH,t_rad_spec,h_rad_spec,cool_tot,heat_tot,cool_com,heat_com,mu,aexp,n_spec,0)
...
!=======================================================================
  ...
  real(kind=8)::fa,fb,fprimea,fprimeb,alpha,beta,gamma,LAGN,cool_com_eps
  ...
  do while(n>0)
     ...
     do i=1,n
        ...
        if(facT.le.logT2max)then
           ...
           !  replace Compton heating and cooling terms
	   ! interpolated from table to terms directly calculated
	   ! given LAGN, r.
	   ...
	   ! T is in K
	   ! Define LAGN and r to be cgs
	   ! 
	   ! cool_com must be cgs, too, before taking the log.
	   T(i) = T2(i)*0.61 ! assume mean molecular weight for fully ionized gas
	                     ! not nec. always appropriate, so should
			     ! use self-consistent value given
			     ! ionization state by calling cmp_chem_eq()
	   

	   ! Turn off Compton cooling for SNR simulations. 
	   !cool_com = cool_compton(T(i),LAGN,r(i))
           cool_com = 0.0

	   T_eps = 10d0**(log10(T(i)) + 0.01)
	   cool_com_eps = cool_compton(T_eps,LAGN,r(i))

	   !cool_com_prime = (cool_com_eps-cool_com)/(T_eps - T(i))
           cool_com_prime = 0.0
	   ...
	   !  neglect Compton heating, for now.
	   heat_com = 0.0
	   heat_com_prime = 0.0
           ...
           !  this is cgs for high-temperature free free, so
	   ! cooling and heating rates in this function must be cgs.
...
!=======================================================================
  ...
  do i_T = 1,nbin_T
     ...
     call cmp_cooling(T2,nH,t_rad_spec,h_rad_spec,cool_tot,heat_tot,cool_com,heat_com,mu,aexp,n_spec,1)
     ...
     call cmp_cooling(T2_eps,nH,t_rad_spec,h_rad_spec,cool_tot_eps,heat_tot_eps,cool_com_eps,heat_com_eps,mu_eps,aexp,n_spec_eps,0)
...
!  if(lTT.ge.temperature_sd93(91))then
  if(lTT.ge.temperature_cc07(91))then
     metal_tot=1d-100 !  no metal lines above T>10^8.5 K, I think
     metal_prime=0d0
...
!      if(lTT.ge.temperature_sd93(1))then
      if(lTT.ge.temperature_cc07(1))then
        !  cooling between 10^4 K and 10^8.5 K.
...
!                    & + excess_prime_sd93(iT)*(temperature_sd93(iT+1)-lTT)/deltaT
     ...
     !
     !  neglect low-T metal line cooling for SNR problem
     lcool2 = -100d0
     lcool2_prime = 0d0
...
!=======================================================================
subroutine cmp_cooling(T2,nH,t_rad_spec,h_rad_spec,cool_tot,heat_tot,cool_com,heat_com,mu_out,aexp,n_spec,switch)
!=======================================================================
  implicit none
  ...
  do while (err_mu > 1.d-4 .and. niter <= 50)
     ...
     err_mu=ABS(err_mu)
     niter=niter+1
     if(switch>0 .and. 0.8<nH .and. nH<1.2 .and. 6000<T2*0.6 .and. T2*0.6<9000)then
        write(*,*)T2,T,mu,T2*0.6,T/mu*0.6
     end if
  end do
  if (niter > 50) then
  ...
  ! don't use tabulated Compton terms.
...
!=======================================================================
function cool_compton(T,LAGN,r)
!=======================================================================
  implicit none
  real(kind=8) ::cool_compton,T
  real(dp):: LAGN,r,Uph

  !cool_compton=5.406D-36*T/aexp**4 
  
  ! Non-relativistic inverse Compton.
  !
  ! energy/(per e * time), cgs.
  Uph = LAGN/(4.0*pi*r**2.0*clight)

  cool_compton = (4.0*kB*T/(me*clight))*sigmaT*Uph
