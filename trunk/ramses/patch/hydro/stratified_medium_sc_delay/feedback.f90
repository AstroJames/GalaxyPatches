...
! and kinetic energy when a SNR explodes.
  ...
  real(dp)::alpha,rcool,rrise,rnod,rbreak,delay
  ...
  !if(t<1.0d7*365.*24.*3600./scale_t)then
  !   delay=0.0 ! 1.0d6*365.*24.*3600./scale_t
  !else
  !   delay=1.0d7*365.*24.*3600./scale_t
  !end if
  delay=min(1.0d7*365.*24.*3600./scale_t*(t*scale_t/(2.0d7*365.*24.*3600.))**2,1.0d7*365.*24.*3600./scale_t)
        ...
        do ind=1,twotondim
           ...
           do i=1,ngrid
              if(ok(i))then
                 ...
                 if(n_expl>0)then
                    ! Initialize time of explosion 10 Myr in the future 
                    if (uold(ind_cell(i),ndim+4)==1.0d20) then ! for cells that don't have an explosion
                       uold(ind_cell(i),ndim+4)=t+delay
                       uold(ind_cell(i),ndim+5)=1.0
                    else ! for cells that already have an explosion
                       if(uold(ind_cell(i),ndim+4)<1.0d20) then
                          uold(ind_cell(i),ndim+5)=1.0 
                       end if
                    end if
                 end if
                 if (t >= uold(ind_cell(i),ndim+4)) then
                    num_snr_local(isnr)=int(uold(ind_cell(i),ndim+5))
                    xsnr(isnr)=x+dx_loc/2
                    ysnr(isnr)=y+dx_loc/2
                    zsnr(isnr)=z+dx_loc/2
                    isnr=isnr+1
                    uold(ind_cell(i),ndim+4)=1.0d20
                 end if
              endif
           end do
        end do
  ...
  if (n_expl > 0) then
     ...
     do i=1,nrandom
        ...
        if(random_expl(i)>0.and.myid==1)then
           ...
           !rrise=9.19*3.08d18/scale_l*(rhocool/scale_nH/100.)**(-0.439)
           if (rc<rcool) then
              E_SN_th=7.1d50 !*dble(random_expl(i))
           else 
              if (rcool <= rc .and. rc < rrise) then
                 E_SN_th=7.1d50*(rc/rcool)**alpha 
              else
                 ...
                 if(ok(i))then
                    ...
                    do isnr=1,nrandom
                       ...
                       if(drr.lt.rmax2.and.random_expl(isnr)>0)then                 
                          ...
                          M_ej = 5.97*1d33 !*dble(random_expl(i)) ! 3 Msun, cgs
