...
! and kinetic energy when a SNR explodes.
  ...
  if(nblast_current_internal.lt.nsnr .and. & 
       & random_expl(nblast_current_internal+1).lt. & 
       & (t-dble(nblast_current_internal)*tout(noutput)/dble(nsnr))*dble(nsnr)/tout(noutput))then 

     if(marker(nblast_current_internal+1).eq.0)then
        
        if(myid==1)write(*,*)'SN explosion, E_th, E_kin (cgs) =',E_SN_th,E_SN_kin
                 ...
                 do i=1,ngrid
                    if(ok(i))then
                       ...
                       if(drr.lt.rmax2)then
                          ...
                          vri = (3.0*E_SN_kin/(2.0*pi*(rho_ej)*rc**3.0)/(scale_v**2.0*scale_d*scale_l**3.0))**0.5
                          ...
                          uold(ind_cell(i),1)=uold(ind_cell(i),1)+rho_ej
                          uold(ind_cell(i),2)=uold(ind_cell(i),2)+rho_ej*vxi
                          uold(ind_cell(i),3)=uold(ind_cell(i),3)+rho_ej*vyi
                          uold(ind_cell(i),4)=uold(ind_cell(i),4)+rho_ej*vzi
                          uold(ind_cell(i),5)=uold(ind_cell(i),5)+0.5*rho_ej*(vxi**2+vyi**2+vzi**2)+Pi_/(gamma-1.0d0)
                       endif
                    endif
                 end do
