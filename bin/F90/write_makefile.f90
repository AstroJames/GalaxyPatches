subroutine output_makefile(filename)
  ...
  write(ilun,format)"PATCH = ../patch/hydro/stratified_medium_fx_NSM"
  write(ilun,format)"GRACKLE = 0"
  write(ilun,format)"EXEC = ramses_"
  write(ilun,format)"#############################################################################"
  ...
  write(ilun,format)"F90 = mpiifort -g"
  write(ilun,format)"FFLAGS = -xAVX -g -traceback -fpp -fast -cpp -fast $(DEFINES) -DNOSYSTEM"
  write(ilun,format)""
  ...
  write(ilun,format)"VPATH = $(shell [ -z $(PATCH) ] || find $(PATCH) -type d):../$(SOLVER):../aton:../hydro:../pm:../poisson:../amr:../io"
  write(ilun,format)"#############################################################################"
  write(ilun,format)"# All objects"
  write(ilun,format)"MODOBJ = amr_parameters.o amr_commons.o random.o pm_parameters.o pm_commons.o poisson_parameters.o dump_utils.o"
  write(ilun,format)"ifeq ($(GRACKLE),1)"
