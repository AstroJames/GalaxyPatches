...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  do ind=1,twotondim
     ...
     if(delayed_cooling)then
        ...
        ! Davide Martizzi: introduce cooling time formula from Martizzi et al. 2015
        do i=1,nleaf
           t_blast = 0.666*3.5d3*(365.*24.*3600.)*(uold(ind_leaf(i),1)*scale_nH/100.)**(-0.53)
           if(metal) t_blast = t_blast*(MAX(uold(ind_leaf(i),imetal)/0.02,0.01))**(-0.17)
