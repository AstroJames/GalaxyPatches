...
subroutine cmpdt(uu,gg,dx,dt,ncell)
  ...
  ! Davide Martizzi: take cooling time into account too
  ! a very raw approximation of the cooling time is used here
  do k = 1,ncell
     T2 = uu(k,ndim+2)**2/gamma*scale_T2
     if (T2 .lt. 7.69d3) then
        lambda = 1.0d-25
     end if
     if (7.69d3 .le. T2 .and. T2 .lt. 2.0d7) then
        lambda = 1.0d-22
     end if
     if (T2 .ge. 2.0d7) then
        lambda = 1.42*1d-27*sqrt(T2)*1.1
     end if
     dtcool = 1.38d-16*T2/(uu(k,1)*scale_nH)/lambda/scale_t/(gamma-1)
     dt = min(dt,dtcool)
  end do
