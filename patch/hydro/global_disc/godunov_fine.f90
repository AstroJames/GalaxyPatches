...
subroutine add_gravity_source_terms(ilevel)
  ...
  do ind=1,twotondim
     ...
     do i=1,active(ilevel)%ngrid
        ...
        if(ndim>0)then
           u=u+f(ind_cell,1)*fact+fana(ind_cell,1)*fact ! Davide Martizzi
           unew(ind_cell,2)=d*u
        endif
        if(ndim>1)then
           v=v+f(ind_cell,2)*fact+fana(ind_cell,2)*fact ! Davide Martizzi
           unew(ind_cell,3)=d*v
        end if
        if(ndim>2)then
           w=w+f(ind_cell,3)*fact+fana(ind_cell,3)*fact ! Davide Martizzi
           unew(ind_cell,4)=d*w
        endif
        e_kin=0.5*d*(u**2+v**2+w**2)
  ...
  do i1=i1min,i1max
     ...
     do i2=i2min,i2max
        ...
        if(poisson)then
           do idim=1,ndim
              do i=1,nexist
                 gloc(ind_exist(i),i3,j3,k3,idim)=f(ind_cell(i),idim)+fana(ind_cell(i),idim) ! Davide Martizzi
              end do
              ! Use straight injection for buffer cells
              do i=1,nbuffer
                 gloc(ind_nexist(i),i3,j3,k3,idim)=f(ibuffer_father(i,0),idim)+fana(ibuffer_father(i,0),idim) ! Davide Martizzi
              end do
           end do
        end if
