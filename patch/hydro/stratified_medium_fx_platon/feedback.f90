...
! and kinetic energy when a SNR explodes.
  ...
  real(dp)::rcool,mcool,a,b,c,d,f,Z_scaled,rho_scaled
  ...
  do while (nblast_current_internal.lt.nsnr .and. & 
     ...
     if(marker(nblast_current_internal+1).eq.0)then
        ...
        if(metal)Z_average=Z_average/weight

        rmax=rc/aexp
        rmax2=rmax*rmax
        if(myid==1)write(*,*)'Injection radius ',rmax
        
        if(myid==1)write(*,*)'Average density ', rho_average

        if(myid==1)write(*,*)'Average metallicity (solar units)', Z_average/0.02
        ...
        rcool = (3.0/4.0/3.14159*mcool/rho_average/scale_d )**0.3333 ! cgs
        rcool = rcool/scale_l ! code units
        ...
        if (rmax>rcool) then
           ...
           Z_scaled = Z_average/0.02
           P_SN_rad = a+b/(1+((Z_scaled)/c)**d)*f**(-log10(rho_scaled))
           P_SN_rad = P_SN_rad*(rmax/rcool)**1.5 
           E_SN_th = 7.1d50
        end if
