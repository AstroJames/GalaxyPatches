...
! and kinetic energy when a SNR explodes.
  ...
  rmax=rc/aexp ! Not sure about aexp, no problem for cosmo=.false..
  rmax2=rmax*rmax
  ...
  if(nblast_current_internal.lt.nsnr .and. & 
     ...
     if(marker(nblast_current_internal+1).eq.0)then
        ...
        if(myid==1)write(*,*)'SN explosion, E_th, P_rad (cgs) =',E_SN_th,P_SN_rad
                 ...
                 do i=1,ngrid
                    if(ok(i))then
                       ...
                       if(drr.lt.rmax2)then
                          ...
                          ! Radial velocity of the gas within rc, so that radial momentum
                          ! is P_SN_rad. To be used for energy injection in kinetic form.
                          vri = 3.0*P_SN_rad/(4.0*pi*(rho_ej+rho_average)*rc**3.0)/(scale_v*scale_d*scale_l**3.0)
