...
!! We need access to the uold array 
    ...
    real(dp)::dx, dx_loc, vol_loc, dm, nx_loc, u_sq, dens, z, skip_loc, P, c_s
    real(dp)::scale,scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v, scale_E
    ...
    real(dp) :: lmin_dens, lmax_dens, gmin_dens, gmax_dens
            ...
            do ind = 1, twotondim
                ...
                do i = 1, ngrid
                    if(ok(i))then ! Only add values from leaf cells 
                        ...
                        ! Momenta along x, y, z:
                        do j = 1,3
                            lsum(2+j) = lsum(2+j) + uold(ind_cell(i),1+j)*scale_v*dm 
                        end do
                        ...
                        lsum(8) = lsum(8) + 0.5*dm*u_sq*vol_loc ! Kinetic
                        lsum(9) = lsum(9) + f(ind_cell(i),3)*(scale_l/scale_t**2)*dm*z*vol_loc ! Potential
                        lsum(10) = lsum(10) + uold(ind_cell(i),5)*scale_E*vol_loc
                        
                        P = uold(ind_cell(i),5)*scale_E
                        do j = 2,4 ! Loop on velocities
                            lsum(10) = lsum(10) - 0.5*(uold(ind_cell(i), j)*scale_v)**2*dm*vol_loc
                            P = P - 0.5*(uold(ind_cell(i), j)*scale_v)**2*dm
                        enddo

                        lsum(11) = lsum(11) + uold(ind_cell(i), 5)*scale_E*vol_loc ! Kinetic + internal

                        ! Velocities
                        u_sq = 0.0
                        do j=1,3 ! Loop on velocities
                            lsum(11+j) = lsum(11+j) + uold(ind_cell(i),1+j)*scale_v
                            lsum(14+j) = lsum(14+j) + (uold(ind_cell(i),1+j)*scale_v)**2*vol_loc
                            u_sq = u_sq + (uold(ind_cell(i), 1+j)*scale_v)**2
                        enddo 

                        lsum(18) = lsum(18) + u_sq*vol_loc
                        lsum(19) = lsum(19) + u_sq**2*vol_loc
                        
                        ! Mach number 
                        c_s = gamma*P/dens
                        lsum(20) = lsum(20) + (u_sq)**(1/2)/c_s*vol_loc
                        lsum(21) = lsum(21) + u_sq/c_s**2*vol_loc 
...
#ifndef WITHOUTMPI
    gsum = 0.0d0
    call MPI_REDUCE(lsum, gsum, 26+1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
    gmin_dens = 0.0d0
...
if (myid == 1) then !Only root cpu writes to file 
    ...
    iq = iq+1; name_out(iq) = 'rms_u_sq';       quant_out(iq) = sqrt(gsum(19)/gsum(1))
    ...
    if ((isFirst) .AND. (nrestart .NE. 0 .or. istat .NE. 0)) then 
        ...
        do i = 1, iq
            ...
            write(tmp_str, '(A)') '#'//trim(tmp_str)//'_'//trim(name_out(i))//',' ! quantity name 
    ...
    write(funit, '(1x,'//trim(tmp_str)//'(ES12.5, ","), ES12.5)')quant_out(1:iq)
