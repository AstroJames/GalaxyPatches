subroutine file_descriptor_hydro(filename)
  ...
  ! Passive scalars with specific names
  if(metal)then
     ivar=imetal
     write(ilun,'("variable #",I2,": metallicity")')ivar
  endif
  if(delayed_cooling)then
     ivar=idelay
     write(ilun,'("variable #",I2,": delayed_cooling")')ivar
  endif
  if(nvar>=icool)then
     ivar=icool
     ! Note: cooling_rate is stored as density-weighted (erg cm^-3 s^-1 * cm^-3)
     ! To get actual cooling rate, divide by density in post-processing
     write(ilun,'("variable #",I2,": cooling_rate")')ivar
  endif
  ! Any remaining passive scalars
  do ivar=icool+1,nvar
