subroutine hydro_flag(ilevel)
  ...
  ! Modified to support forced refinement on restart for resolution increase
  ! -------------------------------------------------------------------
  ...
  ! Check if we should force refinement on restart
  ! This is activated when:
  ! 1. We are restarting (nrestart > 0)
  ! 2. This is the first step after restart (nstep_coarse == 0 or 1)
  ! 3. The flag force_refine_on_restart is set
  force_refine_restart = .false.
  if(nrestart > 0 .and. nstep_coarse <= 1 .and. force_refine_on_restart) then
     force_refine_restart = .true.
     if(myid==1 .and. ilevel==levelmin) then
        write(*,*) 'FORCING REFINEMENT ON RESTART at level', ilevel
     endif
  endif

  ! If forcing refinement, flag all cells at this level
  if(force_refine_restart) then
     ...
     nflag=nflag+ncache*twotondim
     return
  endif

  ! Standard refinement checks
  if(    err_grad_d==-1.0.and.&
       & err_grad_p==-1.0.and.&
       & err_grad_u==-1.0.and.&
       & jeans_refine(ilevel)==-1.0 )return

  ! Loop over active grids for standard refinement
  ncache=active(ilevel)%ngrid
  do igrid=1,ncache,nvector
  ...
  ! Make virtual cells and boundaries
