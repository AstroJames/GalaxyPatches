...
subroutine condinit(x,u,dx,nn,ln_d)
  ...
  do i=1,nn
    ...
    if (dctr<rc) then
      ...
      q(i,ip) = Pi_
      if(metal) then 
         q(i,6)=z_ave*rhocool/(rhocool + rho_ej)+yield_cc_Fe*rho_ej/(rhocool + rho_ej) ! customized metallicity value.
         q(i,7)=z_ave_alpha*rhocool/(rhocool + rho_ej)+yield_cc_alpha*rho_ej/(rhocool + rho_ej)
         q(i,8)=z_ave_rp*rhocool/(rhocool + rho_ej)+yield_cc_rp*rho_ej/(rhocool + rho_ej)
      end if
    else 
    ...
    if(metal) then
       q(i,6)=z_ave ! customized metallicity value.
       q(i,7)=z_ave_alpha
       q(i,8)=z_ave_rp
    end if
