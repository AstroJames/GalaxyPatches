module amr_parameters
  ...
  real(dp)::yield=0.0D0 !yield used for star formation processes
  real(dp)::yield_Ia_Fe  =0.0D0     ! SNe Ia Iron yield
  real(dp)::yield_cc_Fe = 0.0D0 ! SNe CC Iron yeild 
  real(dp)::yield_NSM_Fe = 0.0D0  ! NSM yeild Iron yeild
  real(dp)::yield_Ia_alpha = 0.0D0
  real(dp)::yield_cc_alpha = 0.0D0
  real(dp)::yield_NSM_alpha = 0.0D0
  real(dp)::yield_Ia_rp = 0.0D0
  real(dp)::yield_cc_rp = 0.0D0
  real(dp)::yield_NSM_rp = 0.0D0
  ...
  real(dp)::z_ave  =0.0D0     ! Average metal abundance of iron peak elements
  real(dp)::z_ave_alpha = 0.0D0 !Average metal abundance of alpha elements
  real(dp)::z_ave_rp =0.0D0   ! Average metalicity of r-process elements
  ...
  ! CC parameters
  real(dp)::RandEXP=0.5
  real(dp)::rhocool, Pc, rc, xwind, ywind, zwind
  real(dp)::vin,Mdot_in,Tin_K,logLAGN,alph,r0,h,gam
  real(dp)::E_SN_th, P_SN_rad, E_SN_kin, sn_rate, Ia_vs_NSM,conversion_NSM, conversion_Ia, t_delay_NSM, H_NSM, t_delay_Ia, H_Ia
  character(LEN=160)::medium_type, grid_file, random_file
