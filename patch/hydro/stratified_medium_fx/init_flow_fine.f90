! set NX_LN as preprocessor variable: need to also set in condinit.f90
#define NX_LN 512
...
subroutine init_flow_fine(ilevel)
  ...
  real*4,dimension(1:NX_LN**3)::ln_d !  hard coding ln_d size
  ...
  !
  !  If reading ICs from a file, condinit is not used! 
  !-------------------------------------------------------
  else
     !  if I can make array accessible in condinit, then 
     ! I could read in lognormal density field here, just once.
    
     if (medium_type=='fractal' .or. medium_type=='disk_fractal') then
       ! fractal cloud assumes 3D

       ! Read pre-computed density field.
       !write(*,*) grid_file

       open(unit=1,file=grid_file,form='unformatted',access='stream')
       
       !write(*,*) 'after opening grid_file'
       
       do i=1,(NX_LN**3) !  hard coding nx_ln
         read(1) ln_d(i)
       
          !write(*,*) i
       end do

       !write(*,*) 'just before close statement'

       close(1)

       !write(*,*) 'after ln_d loop'

       ! Want to pass ln_d to condinit

     endif

     open(unit=1,file=random_file,status='old')
     do i=1,nrandom
        read(1,*)random_expl(i),x_expl(i),y_expl(i),z_expl(i)
     end do
     close(1)
     ...
     do igrid=1,ncache,nvector
	   ...
	   !  pass ln_d to condinit
	   call condinit(xx,uu,dx_loc,ngrid,ln_d)
