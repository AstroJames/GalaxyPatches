subroutine courant_fine(ilevel)
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim        
        ...
        do ivar=1,ndim
           do i=1,nleaf
              eint_loc=max(eint_loc-0.5d0*uu(i,1+ivar)**2/uu(i,1)*vol,uu(i,1)*vol*smallc**2)
           end do
        end do
...
subroutine wind_fine(ilevel)
  ...
  do ind=1,twotondim
     ...
     iy=(ind-1-4*iz)/2
     ix=(ind-1-2*iy-4*iz)
     if(ndim>0)xc(ind,1)=(dble(ix)-0.5D0)*dx !-xwinds
     if(ndim>1)xc(ind,2)=(dble(iy)-0.5D0)*dx !-ywinds
     if(ndim>2)xc(ind,3)=(dble(iz)-0.5D0)*dx !-zwinds
  end do
  ...
  do igrid=1,ncache,nvector
	...
	do i=1,ngrid
           ...
           dctr = ((xcell-xwinds)**2.0 + (ycell-ywinds)**2.0 + (zcell-zwinds)**2.0)**0.5
	   ...
	   ! Spatial condition, then add energy.
	     ...
	     vx_BC = (xcell - xwinds)*vin/dctr
	     vy_BC = (ycell - ywinds)*vin/dctr
	     vz_BC = (zcell - zwinds)*vin/dctr
