...
!=======================================================================
subroutine solve_cooling(nH,T2,zsolar,boost,dt,deltaT2,ncell,LAGN,r,z)
!=======================================================================
  use poisson_parameters, ONLY: gravity_params
  implicit none  
  integer::ncell
  real(kind=8)::dt
  real(kind=8),dimension(1:ncell)::nH,T2,deltaT2,zsolar,boost,T,r,z
  ...
  real(dp)::SNrate,t_s,boost_SN,dcgs,ddyn,a1,a2,z0,heating_peh
  ...
  do while(n>0)
     ...
     do i=1,n
        ...
        if(facT.le.logT2max)then
           ...
           ! The SN rate is computed cell-by-cell as in the feedback routine.
           ! Compute expected no. of SN per cell
           boost_SN=100.
           dcgs=nH(ind(i))/scale_nH*scale_d/0.76 ! in cgs
           ...
           z0=z0*3.08d21 ! cm
           z=z-boxlen/2.
           ddyn= 1.0/(4.0*3.14159*6.66d-8)*(a2+a1/sqrt((z(i)*scale_l)**2+z0**2)- &
                & a1*z(i)**2/((z(i)*scale_l)**2+z0**2)**1.5) ! in cgs
           ! Star formation time
           t_s=sqrt(3.0*3.14159/16.0/6.66d-8/ddyn)*boost_SN ! in cgs
           ! the star formation time is set by the external potential generating the force.
           ! SN rate in units of 100 kpc^-3Myr^-1
